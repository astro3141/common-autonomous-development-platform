...
**Workflow Plane** (reference: Microsoft Conductor; replaceable commodity). Owns roles, routing, retry, checkpointing, run context, and the DAG — everything Spec v0.4 §8 items 2–5 already disclaim. It is a *reference*, not a constitutional citizen: any engine that can call the effect boundary (§4) qualifies, exactly as TD §7.1 treated Temporal in v0.4. Its history and events are never authority (TD §7.1: "Temporal history is never read as authority; kernel rows are" — the rule survives verbatim with the product name swapped out). Probe discipline for the reference choice: the 2026-09-07 container probes recorded in the withdrawn attempt (the `base_url` model-call seam on the OpenAI chat-completions wire, the closed provider literal, `ScriptExecutor` inheriting the parent environment) remain valid *measurements* even though the section that made them constitutional is withdrawn; everything else about Conductor — including its `WORK_START` target-adapter contract (dispatch identity, duplicate-start behaviour, authoritative run lookup) — stays UNKNOWN until probed, per the TD §17.2 rule. None of it is CADP contract either way; that is the point of this plane.

**Execution Plane** (a CADP *product service*, not constitutional territory). This is the surface machinery the v0.4 TD already built and measured: the surface broker (`cadp/product/surfaceBroker.ts`), the closed provider registries and measured profiles of TD §17 (`cadp/product/workerProviders.ts`, `reviewProviders.ts`, `planProviders.ts`), the isolation postures of `cadp/product/isolation.ts`, the auth-injection discipline of TD §17.3, and the session scanning / observed-model-and-effort evidence of TD §17.2 and §19. In v0.5 it gets its own declarative contract: an **ExecutionRequest** of profile, provider, and input digests, returning artifacts and honest surface evidence. Model-subscription custody (the `oauth_env` / `auth_files` machinery of TD §17.3) is this plane's *internal* discipline. The Execution Plane is NOT part of the authority boundary and is optional per deployment — a deployment that runs surfaces some other way loses nothing constitutional, only this product service.
...
**Evidence requirements are bound to kernel subjects, not to workflow lifecycle.** The proof that the thin boundary loses no authority-critical evidence is already in the v0.4 corpus, and the bindings differ by kind. `REVIEW` and `VERIFICATION` are candidate-bound: TD §8.3's review-to-effect subject equality (C41) makes the *evaluator* compare the `REVIEW` envelope's bound candidate against the sealed material's `head_sha`, and TD §18.3 binds `VERIFICATION` from `verifier:github-actions` to the exact candidate sha with `produced_at = claim.completed_at`. `BACKEND_EXECUTION` is **run-bound, not candidate-sha-bound**: its subject is `work_run_ref + step` (TD §9.2's evidence table), scanned from the surface's own session logs (TD §8.1/§17.2), and it reaches the candidate-specific `PR_CREATE`/`PR_MERGE` gates only by being named in the sealed effect's `evidence_refs` (TD §19.1) — a K4 binding to the governed effect, not a consumption of workflow state. What all three share is what the boundary argument needs: every binding is to a kernel-recorded subject — a candidate sha, a governed work run, a sealed effect — and no gate in the reference policy consumes an orchestrator lifecycle event as evidence. So removing the orchestrator from the constitutional surface removes nothing a gate depends on.
...
- **Spec v0.4 and its TD remain the valid record of the current generation.** No patching-as-continuity: v0.5 is authored as a successor, the same rule Spec v0.4 §12.1 applied to v0.3 (in translation: v0.3 and TD v1.5 are not v0.4's architecture authority — historical evidence or commodity adapter implementation input only).

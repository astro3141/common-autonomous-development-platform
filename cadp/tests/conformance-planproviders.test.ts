...
test("claude planner retains the byte-identical argv, auth method, and identity class", () => {
  ...
  const prompt = "You are planning bounded autonomous work";
  assert.deepEqual(planArgv(DEFAULT_PLAN_PROVIDER, prompt), HISTORICAL_CLAUDE_ARGV(prompt));
  assert.deepEqual(planArgv(resolvePlanProvider("claude"), prompt), HISTORICAL_CLAUDE_ARGV(prompt));
  ...
  // Reading the checkout stays allowed — the planner is proposal-only, not write-blind.
  assert.doesNotMatch(tools ?? "", /(?:^|,)Read(?:,|$)/u);
});

test("plan argv template expands the PLAN_PROMPT sentinel and no other token", () => {
  const prompt = "INTENT: decompose this\n\nReply with EXACTLY one JSON object";
...
test("§8.4 identity_class.product matches the registered planner", () => {

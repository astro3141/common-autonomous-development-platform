...
export interface SealRequestBody extends RequestDraft {
  readonly allocation_tuple?: AllocationTuple;
}
...
export class Ingress {
  ...
  #assertTupleDigest(allocation: AllocationBinding, tuple: AllocationTuple): void {
    if (sha256Hex(jcs(tuple)) !== allocation.allocation_binding_digest) {
  ...
  #assertFirstSealBinding(
    ...
    tuple: AllocationTuple | undefined,
    active: ActivePolicy,
  ): void {
    ...
    this.#assertTupleDigest(allocation, tuple);
    ...
    for (const projection of mapping.binding_projection) {
      ...
      if (bound.length === 0 || bound[0]!.object_id !== (tuple as Record<string, unknown>)[projection.tuple_field]) {
        throw new IngressRejection(

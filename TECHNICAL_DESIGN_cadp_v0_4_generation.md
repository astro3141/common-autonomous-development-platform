...
| C41 | review-to-effect provenance continuity (end-to-end; Control intake `issuecomment-5518636858`, U2 closure criterion) | one development scenario proven as a chain of existing K1–K7 checks, no new record: exact `REVIEW`/`HUMAN_DECISION` envelope (`subject_bindings`/`scope` = candidate SHA A, `envelope_digest` E) → `AdmissionInputV1` naming E → `PolicyDecisionV1` bound to that `input_digest` → `EffectRequestV1` whose material names candidate A / `expected_head_sha` A → pre-K6 precondition + recheck #3/#5 → K6 → target-authoritative receipt. Falsifications: (1) reviewer approves A, candidate ref/PR head moves to B, review(A) reused; (2) Human approves merge of A, `PR_MERGE` material re-sealed for B, decision(A) reused; (3) review body says APPROVE but the producer is not in `identity_registry` / reviewer identity cannot be established; (4) K6 admitted for A but the target receipt (`head.sha`, `merge_commit_sha` ancestry) corresponds to B (fault-injected); (5) same PR number, candidate rebased to byte-different B, old review(A) presented | (1) recheck #3 (`current_revision ≠ bound A`) → refusal, PR merge delta 0; (2) `scope.material_digest`/`candidate_sha` ≠ new request → refusal at #5 (`DECISION_INPUT_MISMATCH` if the old input is presented), delta 0; (3) Ingress rejects the envelope (unregistered principal) or `identity_class` derives to nothing → required-fact `UNKNOWN` → `DENY`; (4) receipt binding fails → no `COMMITTED`, `UNKNOWN(RECEIPT_UNBOUND)` + `RECEIPT_MATERIAL_MISMATCH` incident + scope hold; (5) `subject_bindings` are `repo_id + commit_id`, never PR number — review(A) does not bind B → refusal, delta 0. Positive leg: with A unchanged the chain admits once and the receipt's `head.sha == A`; the conformance report lists the exact `evidence_id`/`input_digest`/`decision_id`/`admission_id`/`outcome_id` tuple so the continuity is a projection of stored K1–K7 rows, not a new authority record |
...
Guard-bite check (from #89): for C1, C2, C3, C6, C9, C10, C11, C20, C22, C27, C28, C30, C35, C38, C39, C40, C41 the test additionally removes the corresponding kernel check and asserts the prohibited effect **does** occur (delta 1). A control whose removal changes nothing is reported as defence-in-depth, not as load-bearing.
...
| U2 | Human reviewer decision provenance equivalence to machine reviewer (#90 unresolved). Reference: Human decisions enter only via SSO path A; human GitHub reviews are `REVIEW` evidence. Not yet measured; **C41 is the closure criterion** (end-to-end continuity proven on the reference composition). | none — same envelope contract; policy fails closed if a surface cannot bind scope |

...
Those two commands are the repository's checks: `npm test` runs the deterministic suite under
`node --test`, and `npm run typecheck` runs `tsc --noEmit`. There are no others. Both must pass
before a change is proposed — a change that fails either one is not ready to be reviewed.

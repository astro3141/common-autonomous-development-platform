...
/** Validates the exact twelve-field body and returns the envelope with its hash. */
export function sealTaskContract(input: unknown): TaskContractResult {
  const body = validateTaskContractBody(input);
  const envelope = makeEnvelope(TASK_CONTRACT_SCHEMA, 1, body as unknown as CanonicalObject);
...
export function validateTaskContractBody(input: unknown): TaskContractV1Body {
  ...
  exactKeys(body, "", TASK_CONTRACT_BODY_FIELDS);

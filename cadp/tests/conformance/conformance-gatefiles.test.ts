...
test("GF7: the test-directory split — conformance/ is protected whole, ops/ is delegable", () => {
  // Conformance tests are NOT authority themselves; they are the EXECUTABLE ASSURANCE PROJECTION
  // of TD authority, protected to PREVENT WEAKENING OF THE TD ASSURANCE BOUNDARY. So the whole
  // directory routes to a Human — the control projections, the manifest that maps them, the
  // meta-test that keeps the manifest true, and the harness they all run through.
  for (const p of [
  ...
  // Operational contracts carry no assurance-boundary claim: an ordinary candidate may evolve them
  // under a delegated merge. They still RUN in both verifiers — the split changes protection only.
  for (const p of [
  ...
  // The narrowing is a narrowing, not a rename: the blanket rule is gone.
  assert.ok(!GATE_PATH_RULES.includes("cadp/tests/"), "the blanket cadp/tests/ rule must be replaced, not kept alongside the narrowed one");
  assert.ok(GATE_PATH_RULES.includes("cadp/tests/conformance/"), "the conformance directory must be gate machinery");
});

test("GF8: the verifier invocation surfaces are gate machinery, and neither can be evaded", () => {
  // The EXTERNAL verifier's invocation decides what a green check actually attests to, so the
  // workflow file is verification machinery and routes to a Human — named exactly, and not only
  // by the broad `.github/` prefix.
  assert.ok(GATE_PATH_RULES.includes(".github/workflows/cadp-verify.yml"), "the external verifier's invocation must be named as gate machinery");
  assert.deepEqual(touchesGateMachinery([".github/workflows/cadp-verify.yml"]), [".github/workflows/cadp-verify.yml"]);
  // The LOCAL verifier's invocation lives in surfaceBroker.ts, which is already listed — confirmed
  // here rather than duplicated, so `package.json` is not either verifier's selection seam.
  assert.equal(GATE_PATH_RULES.filter((rule) => rule === "cadp/product/surfaceBroker.ts").length, 1, "surfaceBroker.ts is listed exactly once");
  assert.deepEqual(touchesGateMachinery(["cadp/product/surfaceBroker.ts"]), ["cadp/product/surfaceBroker.ts"]);
  // package.json itself stays delegable — it is a developer convenience, deliberately NOT a seam.
  assert.deepEqual(touchesGateMachinery(["package.json"]), []);
});
...
test("GF4: the rule file guards itself and every rule is a real repo path shape", () => {
  ...
  for (const rule of GATE_PATH_RULES) {
    ...
    assert.ok(rule.startsWith("cadp/") || rule.startsWith("**/") || constitutional, `rule ${rule} should be a cadp path, a reviewer-instruction basename, a verification-workflow path, or a constitutional-doc rule`);
  }
});

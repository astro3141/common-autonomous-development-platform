...
test("§19 reviewer argv profiles remain pinned byte-for-byte", () => {
...
test("measured reviewer session-scan capabilities are pinned byte-exactly; effort stays unprobed", async () => {
  ...
  for (const [provider, profile] of Object.entries(REVIEW_PROVIDERS)) {
    ...
    assert.equal(profile.requested_effort, provider === "codex" ? "high" : undefined);
    assert.deepEqual(profile.effort_argv, provider === "codex"

...
|---|---|---|---|---|---|
| K1 `PolicyRefV1` | **published only** by the store adapter inside a `POLICY_ACTIVATE` dispatch (publish-if-absent + activation append in one transaction, §9.4) or by the root genesis procedure; there is no other publication path or API | Ingress at `POLICY_ACTIVATE` recheck: `content_digest` over the CAS bundle bytes (`raw-bytes-1`), `payload_digest`, manifest, `cadp.kernel-config.v1` | `policy_ref` table + `policy_activation` append-only log; bundle bytes in CAS | immutable; activation is a new log row | OPA bundle **is** the policy content; OPA never writes this table |
...
evidence_envelope     + impl columns: received_at, produced_at_authority (SOURCE|INGRESS), produced_at_locator   -- §9.1, outside K2 digests
...
ResolvedAdmissionBundle = {
  ...
  ingress_meta         : per evidence_id { identity_class, produced_at_authority, received_at }   (§9.1; not part of K2)
...
Policy expresses separation as predicates over `producer_ref` and the Ingress-derived `ingress_meta.identity_class` (vendor/product/account/process_class; §9.1 — derived from `producer_ref` through the active policy's `identity_registry`, never read from the claim): e.g., `producer_ref(REVIEW) ≠ producer_ref(WORK_STEP implementation)` and `identity_class.product ≠`. The kernel supplies the exact identities; it does not rank or score them. A single-product deployment that cannot satisfy the predicate fails closed (the case #90 did not measure).
...
- **Two clocks, two meanings (S1).** `produced_at` (K2 top-level) carries the **source-authoritative** production/observation time when the source reports one (GitHub timestamps, Temporal event time, CI run `completed_at`, IdP `iat`). The adapter submits, next to the draft and **outside** `claim`, a `source_time = { authority: SOURCE | NONE, locator? }` declaration; the Ingress verifies a `SOURCE` declaration by replaying `locator` against the raw source (as for observed backend fields) and records the verified result in the implementation columns `produced_at_authority ∈ {SOURCE, INGRESS}` and `produced_at_locator`. When the source reports no authoritative time (`NONE`), the Ingress fills K2's mandatory `produced_at` with its own receipt time and records `produced_at_authority = INGRESS`. The receipt time itself is the implementation column `received_at`. **Rule:** `EVIDENCE_MAX_AGE` and every §4.4 freshness check read `produced_at` only when `produced_at_authority = SOURCE`; `INGRESS` makes the source-freshness requirement `UNKNOWN` and fails closed (C27). The `ResolvedAdmissionBundle` (§5.1) carries `produced_at_authority` and `received_at` per envelope as `ingress_meta`, clearly separated from the envelope. Policy may bound `received_at` only via the explicit `INGRESS_MAX_AGE` predicate, which is not a freshness claim about the fact. These columns share the store's insert-only trust class (§2.4); they are not part of the K2 digest and are reproducible from the raw source via `produced_at_locator`.
- **`identity_class` is derived, not stored (S2).** K2 `producer_ref` is stamped from the authenticated principal (exact match in the active policy's `identity_registry`; unregistered principals cannot submit). `identity_class = {vendor, product, account, process_class}` is **not a field of any record**: the Sealer/PEP derives it at evaluation and recheck time as `identity_registry[producer_ref].identity_class` under the **active** policy and supplies it in the `ResolvedAdmissionBundle` as `ingress_meta.identity_class` per envelope. Adapters and drafts cannot set, override or supplement the class; any class-shaped field in a draft's top level is rejected, and anything inside `claim` is opaque source payload with no kernel meaning (C28). The former draft field `reviewer_identity_class` is removed.
...
submit_evidence(draft, source_time?)                      -> EvidenceEnvelopeV1       (producer_ref stamped; WORK_STEP idempotent, §7.4)
...
Ten calls. There is no task, attempt, batch, pending-decision, profile or **policy-publication** endpoint (a `PolicyRefV1` is published only inside a `POLICY_ACTIVATE` dispatch or by root genesis, §9.4). `admit_and_dispatch` is the only call that can cause an external effect, and it is the only path that inserts `effect_admission`.
...
| `submit_evidence` | `WORK_STEP`, `WORK_BOUND_STOP` | `BACKEND_EXECUTION`, `REVIEW`, `VERIFICATION` (as registered) | kinds as registered (`VERIFICATION`, `TARGET_RECONCILIATION`, `CREDENTIAL_REACH_ATTESTATION`, `TARGET_IMMUTABILITY_ATTESTATION`, `LEGACY_V03_ARTIFACT`) | `HUMAN_DECISION` only | `GENESIS`, `BREAK_GLASS` — **root listener only** |
...
| C27 | re-ingested stale evidence (S1) | a CI run completed 10 days ago submitted now with `source_time.authority = NONE`; policy `EVIDENCE_MAX_AGE(VERIFICATION, 3600)` | `produced_at_authority = INGRESS` → freshness `UNKNOWN` → `DENY required_fact_unknown`; the same run submitted with a `SOURCE` locator to GitHub `completed_at` → age 10 d → `DENY`; a `SOURCE` declaration whose locator does not replay → Ingress rejects; K2 `claim` bytes identical in all cases; PR delta 0 |
| C28 | identity_class self-assertion (S2) | reviewer principal P (registry: product X) submits a draft with a top-level `identity_class.product = Y` and another with the same inside `claim`; separately, the implementer principal submits a review draft | top-level class field rejected; the in-claim value is stored as opaque payload and ignored — `ingress_meta.identity_class` for P is derived as product X from the active registry; independence predicate `product ≠` evaluates on derived classes → `reviewer_is_the_implementer` when P is the implementer; PR delta 0 |
...
| C37 | UNKNOWN envelope stays claim-less | submit a `BACKEND_EXECUTION` draft with `availability = UNKNOWN` | stored envelope has no `claim`/`claim_digest`, has `unknown_reason`; `produced_at_authority`/`received_at` exist only as impl columns; K2 schema validation passes |
...
Guard-bite check (from #89): for C1, C2, C3, C6, C9, C10, C11, C20, C22, C27, C28, C30, C35 the test additionally removes the corresponding kernel check and asserts the prohibited effect **does** occur (delta 1). A control whose removal changes nothing is reported as defence-in-depth, not as load-bearing.

...
| `REVIEW` | GitHub PR review / second-surface reviewer | `repo_id + commit_id` | `cadp.review.v1` `{verdict, body_digest, body_cas_key, reviewer_run_id?}` (`body_digest` is taken over exactly the CAS bytes at `body_cas_key` — a self-verifying pair; identity class derived from `producer_ref`, §9.1) | `INDEPENDENT_OBSERVATION` or `SELF_REPORT` (if producer == implementer) |
...
**REVIEW.** `reviewCandidate` submits `REVIEW` as `reviewer:claude-code` / `reviewer:grok` / `reviewer:codex` with `cadp.review.v1` `{ verdict, body_digest, body_cas_key, reviewer_run_id }` (#259 P0b: the reviewer's full stdout is put in the kernel CAS under the reviewer's own principal and `body_digest` is taken over exactly those bytes, so the pair is self-verifying — fetch by `body_cas_key`, sha256, compare. Stored whole: the 60 000-char cap bounds the reviewer PROMPT, never retained evidence. The claim contract is built and validated by the producing adapter, `cadp/product/reviewEvidence.ts`, because the Ingress holds no claim schema knowledge). The chosen product is the kernel-stamped `producer_ref` (identity_class derived from the active `identity_registry`, never from the claim). The claim carries no observed model and no effort. `brokerReview` calls `runReviewer` with `{ workspace, auth, argv }` only.

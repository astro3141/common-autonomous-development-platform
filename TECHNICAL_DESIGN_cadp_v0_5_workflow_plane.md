...
- The proposer calls `allocate_effect_id` with a versioned canonical tuple (TD v0.4 §7.4 discipline: canonical JSON, closed `purpose` vocabulary, no raw concatenation), schema `cadp.allocation-key.external.v1`: `{ schema, repo_id, base_sha, candidate_sha, purpose }`. `base_sha` is an exact immutable identity component alongside repository identity and `candidate_sha` — without it, two submissions whose candidates sit on materially different base revisions could share an `effect_id`, contradicting K3's binding of one `effect_id` to one `request_digest` over exact material. A submission that reuses an allocation key with a different `base_sha` — or any otherwise-different sealed material — is a K3 conflict and performs no effect (Spec v0.5 K3). The Ingress scopes the derived key by the **stamped** requester principal — two principals proposing the same candidate get distinct effect identities; a retry by the same principal converges on the same `effect_id`, so an orchestrator that re-runs a step never creates a second logical effect. (This replaces the internal tuple's `work_run_ref + step_ordinal` scoping, which presumes run-profile enrollment — §5 refuses that here.)
...
|---|---|---|
| `GIT_PUSH` | `{repo_id, ref = refs/heads/cadp/candidate/<candidate_sha>, new_sha = candidate_sha, expected_old_sha = 0000…, bundle_cas_key}` | The proposer `put_blob`s a `git bundle` of the candidate; the PEP verifies the bundle reproduces **exactly** `candidate_sha` before pushing (TD v0.4 §6.6, §8.1). An external bundle is exactly as untrusted as a worker bundle was; the same verification makes it safe. The sealed request also binds `base_sha` as exact immutable input (rule below); the PEP's ancestry check runs against exactly that admitted `base_sha` (TD v0.4 §6.4). |
| `PR_CREATE` | `{repo_id, base_ref, head_ref = refs/heads/cadp/candidate/<head_sha>, head_sha, title_cas_key, body_cas_key}` | `head_sha = candidate_sha`; write-once candidate ref shape enforced at admission (TD v0.4 §4.4 #14). The sealed request also binds `base_sha` — the exact base revision `base_ref` names at seal time — as exact immutable input (rule below). |
| `PR_MERGE` | `{repo_id, pr_number, expected_head_sha, merge_method}` | `expected_head_sha = candidate_sha`; target-native CAS rejects head drift. The sealed request also binds `base_sha` alongside `expected_head_sha` as exact immutable input (rule below). |

For each of the three effects, `base_sha` and repository identity enter the sealed `EffectRequestV1` alongside `candidate_sha` as exact immutable bindings: `work_bindings` carries them as exact input identity + revision, covered by `request_digest` (Spec v0.5 K3), and the sealed `base_sha` must equal the one the §3.3 allocation tuple names — reuse of an allocation key with a different `base_sha`, like any other material difference under the same `effect_id`, is a K3 conflict and performs no effect. This is the same exact-input discipline TD v0.4 §8.3 applies to the candidate subject in the development composition — equality over K4-bound sealed facts, decided by the evaluator; inequality = requirement unsatisfied, no K6 — extended to the base revision that composition already pins (worker workspace materialized at exactly `base_sha`, bundle ancestry checked against the admitted `base_sha`; TD v0.4 §8.1, §6.4). Policy, reviewers, and the merge decision therefore all evaluate against a pinned base. Any ancestry rule (candidate descends from `base_sha`) is development-composition semantics per Spec v0.5 §7, evaluated by the evaluator over K4-bound facts — never new kernel vocabulary.
...
| `base_sha` exact binding in identity tuple and sealed material; allocation-key reuse conflict | Spec v0.5 K3 (one `effect_id` = one `request_digest`); TD v0.4 §8.3 (exact-binding discipline), §8.1, §6.4 (`base_sha` in the development composition) |

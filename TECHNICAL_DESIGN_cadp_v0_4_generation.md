...
1. verify the signature against `root_public_keys` of the **currently active** policy (the key set the proposed bundle may change is never used to authorize its own introduction), key valid at `created_at`; require `created_at ≤ now < expires_at`;
...
Guard-bite check (from #89): for C1, C2, C3, C6, C9, C10, C11, C20, C22, C27, C28, C30, C35, C38, C39 the test additionally removes the corresponding kernel check and asserts the prohibited effect **does** occur (delta 1). A control whose removal changes nothing is reported as defence-in-depth, not as load-bearing.

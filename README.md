...
cadp/tests/         deterministic suites, split by what they protect:
                    conformance/ = the executable assurance projection of TD
                    authority (C*/P*/FC*/WB* controls + traceability manifest),
                    gate-protected; ops/ = operational contracts only
                    (prompt shapes, provider argv, T* timeout budgets), not
                    gate-protected. Both run in both verifiers.

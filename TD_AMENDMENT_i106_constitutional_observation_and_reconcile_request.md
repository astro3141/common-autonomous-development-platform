...
| Field | Value |
|---|---|
| Status | **DESIGN CANDIDATE — superseding contract, repair round 3 applied; TD_DESIGN_READY (§12)**; TD v2 amendment authority only upon Control scope check + independent Review + Human TD decision/merge |
...
| T1 continuity authority | **Human decision `#106 issuecomment-5540963810` (RESOLVED)**: non-flag-day, explicit versioned policy schema; v1 remains valid with existing semantics; v2 features only after explicit ordinary `POLICY_ACTIVATE` of v2; silent permission change forbidden. §7 conforms to this decision |
...
**§12 integration-edit shape (M7).** The landed TD §12 table has methods as rows and **five caller-category columns** — `workflow / orchestrator`, `worker / reviewer / verifier`, `evidence adapters (CI, target reconciler, deployment control)`, `Human SSO surface`, `root` — i.e. the measured `api.ts` classes `evidence-adapter` and `deployment-control` share one landed column header while `METHOD_REACH` keys them separately. The integration edit rewrites the §12 table at `process_class` granularity exactly as above (one column per class plus root), preserving every landed cell value verbatim, appending the four `observe_*` method rows complete across **every** column, and appending the two new-class columns complete across **every** landed method row. No cell is left implicit: `workflow`, `worker`, `evidence-adapter`, `human-surface`, `deployment-control`, and root are all explicitly `no` for every `observe_*` method — granting `observe_*` to `workflow` is prohibited because it would recreate the B1/B5 read+write co-location this amendment exists to remove.

**Per-principal gating within `observer` (M3).** The `observer` `yes` cells above are class reach; each observer principal is further gated by its registry entry's `record_projection` and optional `observe_scope` (§4.4). Under the default `record_projection: REFS_AND_DIGESTS_ONLY`, the full-record methods `observe_effect` / `observe_evidence` are refused for that principal (`PROJECTION_FORBIDDEN`); only the locator methods `observe_effects` / `observe_evidence_by_subject` are callable.
...
observe_evidence_by_subject(query)
    -> ObservationV1<EvidencePage>
       query = { evidence_kind: string, subject: { authority_ref, namespace, object_id },
                 page_token?: string }
...
**Subject predicate is pinned (M8, normative).** The `observe_evidence_by_subject` query subject is exactly the 3-tuple `(authority_ref, namespace, object_id)` — the same fields the landed subject index stores (TD §3.2) — matched by **exact string equality on all three fields only**. `revision_or_version` and `content_digest` (present in the stored `SubjectBindingV1` record type) **must not be supplied** in this query: the query schema is closed and a request carrying either field is rejected as malformed, not treated as a wildcard, filter, or require-absent predicate. There is no wildcard, prefix, or partial-tuple matching. Digest-level precision is expressed only via `observe_evidence` with `expected_envelope_digest` (exact-ref lookup). This pin is what makes a zero-row `complete: true` result meaningful: the store's index predicate and the contract predicate are the same predicate by construction.
...
ObservationV1<T> = {
  observed_at         : RFC3339 UTC ms      -- Kernel Service clock at the read transaction;
                                            -- PRESENTATION METADATA ONLY, never an ordering input (M2)
  read_authority      : "PRIMARY"           -- fixed in this amendment; see §5.1
  store_authority_ref : string              -- stable identifier of the primary constitutional store
                                            -- instance; equal across all observations served from the
                                            -- same store (reference: the deployment's GENESIS evidence_id)
  snapshot_position   : string (required)   -- monotonic commit position of the read snapshot,
                                            -- totally ordered and non-decreasing across committed writes
                                            -- WITHIN one store_authority_ref; read inside the same
                                            -- read transaction as every row in this response
...
AbsenceBasisV1 = {
  ...
  scope          : <the exact request predicate, echoed verbatim,
                    plus the principal's entitled horizon when scoped (§4.4)>
  complete       : true                     -- an absence basis is NEVER issued with complete = false (§5.2)
  as_of          : { observed_at, snapshot_position, active_policy }   -- of the enclosing ObservationV1;
                                            -- the absence claim is exact as of snapshot_position
}
```

**Ordering rule (M2, normative).** Any consumer that orders or compares two observations — in particular the #96 `STUCK_SUSPECTED` comparison — must order by `snapshot_position` and only between observations with equal `store_authority_ref`; observations with differing `store_authority_ref` are incomparable and any derived ordering claim is `UNKNOWN`. `observed_at` is a wall clock subject to skew and step and **must never** be used to order observations or to upgrade an older snapshot over a newer one. Reference bindings for `snapshot_position`: PostgreSQL — the snapshot's LSN read inside the read transaction; SQLite — a data-version equivalent **derived inside the read transaction from the append-only store itself** (e.g. the sum of per-table max rowids across the constitutional tables: every committed write is an insert, so the value strictly increases with each committed row and no new table or counter row is needed — the §9/M6 "no new store table" claim is preserved).

### 4.3 Snapshot, digest, and completeness semantics (closes T6/T7/T8)

1. **Single-snapshot rule.** Every `observe_*` call executes all of its row reads, including the `active_policy` basis read, inside **one** read transaction against the primary constitutional store (reference: PostgreSQL `REPEATABLE READ` `READ ONLY` transaction; SQLite one deferred read transaction). All rows in one response are therefore mutually consistent as of one store snapshot; `snapshot_position` names that snapshot's commit position (the ordering authority, §4.2), `observed_at` names its wall-clock time (presentation only), and `basis.active_policy` names the constitution active in it. Responses never mix snapshots.
...
5. **Enumeration completeness and policy-bound read safety (M4).** Enumerations are index-ordered (insertion order per the landed indexes) and paged; one page returns at most `observe_max_records_per_query` (§7.1) rows. `complete: true` iff the enumeration exhausted the scope predicate within this response. A truncated page (`complete: false`) carries `next_page_token` (opaque, encoding the exact scan position; valid only for the same scope predicate) and **cannot ground any negative claim** (§5.2). Because each page is its own snapshot, a multi-page enumeration is not one snapshot; the kernel therefore issues `AbsenceBasisV1` only for single-response complete reads, never assembled across pages. A call whose read transaction exceeds `observe_deadline_ms` (§7.1) returns whole-call `UNKNOWN { unknown_reason: "READ_DEADLINE_EXCEEDED" }` — never a partial result, never an absence basis. Both bounds are **policy keys under TD §5.4 ("no implementation defaults"), not build constants**: because `complete` vs `complete: false` is load-bearing for negative inference, two kernel builds under the same active policy must produce the same completeness answer for the same store state; any change to these bounds is a `POLICY_ACTIVATE` with a policy digest difference, visible in every subsequent `basis.active_policy`.
...
### 4.4 Observer scope and content projection (M3)

**Posture, stated explicitly.** `observe_effects` and `observe_evidence_by_subject` are the system's first bulk constitutional reads: an unscoped `observer` can name arbitrary `work_run_ref`s and subjects, and `observe_evidence` returns whole K2 envelopes including potentially sensitive Human/Review/backend claims. This reach is a **strict superset of any landed read reach** and is not a default: it is a deliberate, per-principal policy grant, bounded by the two per-entry fields below (v2 `identity_registry` observer entries; schema in §7.1).

1. **`record_projection: "FULL" | "REFS_AND_DIGESTS_ONLY"`** (optional; **default `REFS_AND_DIGESTS_ONLY`**). Projection is realized as **method gating, not response redaction** — the §4.1 result-type vocabulary stays closed and no redacted/partial record variant exists (F3 closure preserved). Under `REFS_AND_DIGESTS_ONLY` the principal may call only the locator methods (`observe_effects`, `observe_evidence_by_subject`, both returning refs + digests only); the full-record methods (`observe_effect`, `observe_evidence`) are refused with deterministic `PROJECTION_FORBIDDEN`. Under `FULL` all four methods are callable. The #96 operability composition requires exact K2 envelopes, so its reference-policy observer principal is granted `FULL` **deliberately**; the schema default remains least-content.
2. **`observe_scope`** (optional). When absent, the principal's horizon is the whole constitutional store (`ANY` — the deliberate unscoped grant; policy authors grant it knowingly, never by omission of review: the field's absence in a bundle is itself reviewable policy content). When present, it is a non-empty list of scope entries, each one of:

```text
{ kind: "WORK_RUNS_STARTED_BY", started_by_producer_ref: <exact producer_ref string> }
{ kind: "SUBJECT_AUTHORITY",    authority_ref: <exact authority_ref string> }
```

   The principal's **horizon** is the union of its entries: an effect is in horizon iff its work run's `WORK_START` has `requester_ref` equal to a listed `started_by_producer_ref` (same identity-field rule as §6.2; effects outside any work run are in no scoped horizon); a K2 envelope is in horizon iff it carries at least one subject binding whose `authority_ref` is listed. `observe_effects` requires the named run in horizon; `observe_evidence_by_subject` requires `query.subject.authority_ref` listed; the full-record methods require the named record in horizon.
3. **Horizon-uniform responses (no existence oracle).** For a scoped principal, every read behaves **as if the horizon were the entire store**: an out-of-horizon record and a nonexistent record are indistinguishable. Exact-ref reads return `NOT_PRESENT`; `observe_effects` over an out-of-horizon or nonexistent run yields the zero-row complete enumeration, i.e. `NOT_PRESENT` under §5.2 (b). No `FORBIDDEN`-class refusal ever distinguishes existing-but-out-of-scope from absent. Consequently a scoped principal's `NOT_PRESENT` / empty-complete results are **horizon-relative**: `AbsenceBasisV1.scope` echoes the entitled horizon together with the request predicate, and such an absence grounds negative claims only about the entitled horizon (§5.2/§5.3). A product that must make store-absolute negative assertions uses a principal whose horizon covers the asserted scope (`ANY` or a covering registry).

## 5. Read authority, absence, and freshness semantics (D3)

### 5.1 Read-authority guarantee

The observation API is served by the Kernel Service process reading the **primary** constitutional store — the same store, same consistency domain, as admission reads. `read_authority: "PRIMARY"` is a normative claim of exactly that. **No replica, export, cache, or secondary serving path is authorized by this amendment.** A deployment that cannot serve observations from the primary store has no conforming observation surface (fail closed); defining a replica/export watermark contract is explicitly out of scope and would require a new bounded TD round. This resolves the stale-projection question by construction: there is no permitted lagging source, so no replica-freshness watermark is needed or defined. (`snapshot_position`, §4.2, is not such a watermark: it orders primary snapshots relative to each other for consumers comparing observations; it proves no replica currency because no replica exists on this surface.)
...
- v2 `identity_registry` entries with `process_class = "reconcile-requester"` carry a required `entitlements.reconcile_scope` (the per-entry schema is closed: `observer` entries may carry only the §4.4 entitlement fields, and entries of every other class must omit `entitlements` entirely — §7.1):
...
1. **Conclusive short-circuit.** If `E` has admissions and every one is conclusive, refuse `{ accepted: false, reason: "CONCLUSIVE", latest_outcome_ref }`; if `E` has no admission at all, refuse `{ accepted: false, reason: "NO_OPEN_ADMISSION" }`. No probe either way.
2. **Dedupe / coalesce.** If the latest reconciler-authored K7 row across **all open admissions of `E`** has `observed_at` within `reconcile_request_min_interval_s`, return `{ accepted: true, coalesced: true, outcome_ref: <that row> }`. No probe, no new K7 row.
3. **Backoff.** If any reconciler-authored K7 row across **all open admissions of `E`** has `observed_at` within `reconcile_backoff_s`, refuse `{ accepted: false, reason: "RECONCILE_BACKOFF", retry_after_s }`.
4. **Requester budget.** If the count of requester-origin K7 `UNKNOWN` rows for `E` (across all admissions) ≥ `reconcile_request_max_attempts`, refuse `{ accepted: false, reason: "REQUEST_BUDGET_EXHAUSTED" }` — a per-effect ceiling on target-probe consumption; it seals no envelope and changes no constitutional state.
5. Otherwise run one pass (still under both locks): the landed per-open-admission loop — one probe and one K7 row per open admission of `E` — then release `D` and `M(E)` in that order.

**Multi-open-admission rule (normative).** The landed model permits several open admissions per effect (measured: `reconcileEffect` iterates every non-conclusive admission). The gates above are anchored **per effect** — dedupe/backoff on reconciler-authored K7 rows across all open admissions, budget on requester-origin rows across all admissions — so a requester cannot multiply probes by admission; an accepted pass then probes each open admission exactly as landed, and exhaustion accounting stays **per admission** (below).

**Backoff/dedupe anchor (M5, normative).** The only anchor for steps 2–3 is the `observed_at` of reconciler-authored K7 rows of `E`, exactly as stated — there is no reservation row, no request key, no epoch bucket, and **no policy-digest input**: refused requests write nothing and advance nothing, and a `POLICY_ACTIVATE` (of any bundle) does **not** reset or re-key dedupe or backoff. The active policy digest appears in observation/refusal *audit* context only (e.g. `ObservationV1.basis`), never in any dedupe/backoff computation. The first candidate's "policy-owned backoff epoch" and reservation-ledger constructs are **withdrawn** (see M6): accounting rides exclusively on constitutional K7 rows that already exist in the landed store, so no operational table enters the constitutional store contract (TD §3.1/§3.2 untouched) and no §0/§1 store-map change is needed.
...
**Crash-window behavior (read→K7, normative).** If the kernel crashes after an adapter probe but before the outcome row commits, no K7 row exists for that probe; the in-process mutex and advisory lock vanish with the process, and the landed restart scan (TD §3.3) re-reconciles open admissions. The accounting error direction is **conservative by construction**: a lost row means one *fewer* recorded attempt, so kernel-origin exhaustion can only be reached later than the bound, never earlier, and requester budget can only under-count — no crash sequence mints `RECONCILE_EXHAUSTED` early or double-counts a probe (O15).

**Origin-tagged accounting (the reserved floor).** Outcome rows written by the Reconciler carry an exact `observer_ref` discriminator: `<pep_ref>:reconciler` for kernel-originated passes (start scan, dispatch timeout, `UNKNOWN`-write follow-up, kernel-scheduled backoff retries) and `<pep_ref>:reconciler:requested` for requester-originated passes (any accepted `request_reconcile`, whatever the caller class). Matching is exact string equality. The exhaustion rule becomes: **`RECONCILE_EXHAUSTED` is sealed only when kernel-originated `UNKNOWN` rows alone reach `reconcile_max_attempts`** (counted per admission, as landed). Requester-origin rows are honest append-only K7 observations (Spec K7 permits many per admission) but count toward no exhaustion condition. Consequently **no sequence of monitor-controlled requests can force `RECONCILE_EXHAUSTED`**, consume the kernel's reserved reconcile budget, or push an effect onto the Human exception branch; the kernel's own schedule retains the full constitutional floor of `reconcile_max_attempts` attempts regardless of requester behavior.

**Quantified guarantee (M1).** The first candidate's `reconcile_request_reserved_attempts` reserved-floor field is **withdrawn**; no such key exists in v2. In the Review's requested form — "external requests may consume at most `reconcile_max_attempts − reserved` attempts" — this design yields the degenerate strongest bound: external requests consume at most **zero** of `reconcile_max_attempts`, i.e. the effective reserve is the entire budget, strictly inside the Review's required `ceil(reconcile_max_attempts / 2)..reconcile_max_attempts` range. The relocated harm M1 identified (external probes exhausting the target's tolerance rather than the kernel's ledger) is bounded separately and on the trusted side: requester-origin probes per effect are capped by policy-bound `reconcile_request_max_attempts` and rate-shaped by the dedupe/backoff gates above, none of which a requester can widen. (Under an active v2 bundle the landed suffix-match `endsWith(":reconciler")` is replaced by exact equality; `:reconciler:requested` must never be counted by the exhaustion filter. Under v1 the landed filter runs verbatim — §7.2.)
...
data.cadp.schema = "cadp.kernel-config.v2"
  ...
  observe_max_records_per_query     int 1..10000   (required in v2; reference 1000)
  observe_deadline_ms               int 100..60000 (required in v2; reference 5000)
  ...
  "reconcile-requester" (closed per-entry schema):
    process_class = "reconcile-requester" -> REQUIRES entitlements.reconcile_scope (§6.2)
    process_class = "observer"            -> MAY carry entitlements.record_projection
                                             ("FULL" | "REFS_AND_DIGESTS_ONLY";
                                              absent = "REFS_AND_DIGESTS_ONLY", §4.4)
                                             and entitlements.observe_scope
                                             (non-empty scope-entry list, §4.4;
                                              absent = ANY horizon)
    every other class                     -> MUST omit `entitlements`
  Unknown keys anywhere under data.cadp remain rejected.
```

There are no implementation defaults for the new keys (TD §5.4 rule preserved): under a v1 bundle the features they govern are **absent**, not defaulted. (`record_projection`'s absent-means-`REFS_AND_DIGESTS_ONLY` is a **schema default value inside a v2 bundle** — a validated, digest-covered part of the bundle content, activated only by explicit `POLICY_ACTIVATE` — not an implementation default; it exists so the least-content posture is what omission buys, per M3.)

**`observe_max_queries_per_request` is deliberately not adopted (M4 variance, with rationale).** The Review's third suggested key has nothing to govern: every `observe_*` call executes exactly one scope predicate in one read transaction — the contract defines no batch/multi-query request. A config key that no behavior reads would itself violate the §5.4 no-dead-config discipline this repair enforces. If a batch observation request is ever introduced, that TD round must add the key.
...
### 7.4 Continuity authority and future schema versions (T1, Human-decided)

The continuity model of this section is not a Design-side choice: it implements the **Human authority decision `#106 issuecomment-5540963810`** — non-flag-day, explicit versioned policy schema. Point-for-point conformance:

```text
PRE_AMENDMENT_V1_BUNDLE     remains valid with its existing semantics        -> §7.1 (v1 unchanged), §7.2
AMENDED_KERNEL + V1         preserves v1 behavior; v2-only features off      -> §7.2 (byte-for-byte landed; O4)
V2_FEATURE_ACTIVATION       only via explicit ordinary POLICY_ACTIVATE of v2 -> §7.3 step 2
SILENT_PERMISSION_CHANGE    forbidden                                        -> §7.2 fail-closed rule + no-silent-revocation rule
```

**Future versions (normative, per the same decision).** Any future `cadp.kernel-config.v3+` defaults to the same pattern: a backward-compatible versioned schema in which every prior-version bundle remains valid with unchanged semantics and new features activate only by explicit `POLICY_ACTIVATE` of the new version. If a future change **cannot** be expressed backward-compatibly (an incompatible semantic for an existing key, or removal of one), that change is a **STOP → Human migration/flag-day decision**; no TD amendment or implementation may introduce it unilaterally.

## 8. Falsification controls (acceptance item 10)

To be merged into TD §13.1 numbering on TD integration; guard-bite discipline (remove the check, assert the prohibited observable occurs) applies to O1, O2, O3, O4, O6, O7, O8, O10, O11, O13, O16.
...
| O3 | absence and completeness semantics | (i) `observe_evidence` on a nonexistent id; (ii) with `expected_envelope_digest` ≠ stored digest; (iii) `observe_evidence_by_subject` over a subject with rows > `observe_max_records_per_query`; (iv) same scope small enough to complete; (v) exact enumeration: a work run with exactly N effects (counted at the store), `observe_effects` paged to completion | (i)(ii) `NOT_PRESENT` with `AbsenceBasisV1{read_authority: PRIMARY, complete: true}`; (iii) `complete: false` + `next_page_token`, **no** absence basis anywhere in the response; (iv) `complete: true`; (v) the union of pages holds exactly N distinct `effect_refs`, final page `complete: true` — no missing, duplicated, or phantom ref; guard-bite: issue an absence basis on a truncated page → control fails |
...
| O5 | bundle validation, both directions | (i) validate a v2 bundle against the **v1** kernel-config validator (the landed #17 logic); (ii) amended validator on defective v2 bundles: `reconcile-requester` entry without `reconcile_scope`; `entitlements` on a `workflow` entry; empty `observe_scope` list; out-of-bounds `observe_deadline_ms`; an unknown key under `data.cadp`; a `subject` query field beyond the 3-tuple | (i) refused (unknown `schema`) pre-K6: no `policy_ref` row, no activation row, active policy unchanged; (ii) every defective bundle refused pre-K6 with the active policy unchanged — never a partial activation, never a defaulted-in value |
| O6 | in-flight dispatch refusal + fresh-read rule (T3/F4; v2 active) | pause the PEP inside lock D after K6 commit and before the outcome write (C20-style fault injection); issue `request_reconcile` for the same domain during the pause; release; separately: let a dispatch complete conclusively in the window between a request's arrival and its lock acquisition | during pause: `{accepted:false, reason: DOMAIN_BUSY, retry_after_s}`; no reconciler K7 row in the window; after release: an accepted pass proceeds; the post-completion request returns `CONCLUSIVE` (fresh in-lock read), never probes; a request naming an effect with zero admissions returns `NO_OPEN_ADMISSION`, never probes; guard-bite 1: remove the try-acquire of D → the interleaved probe writes an outcome contradicting the dispatch (`OUTCOME_CONTRADICTION` observable); guard-bite 2: evaluate §6.4 steps on pre-lock reads → the stale-eligibility probe runs, proving the fresh-read rule is load-bearing |
...
| O10 | dedupe/backoff determinism + activation independence (M5; v2 active) | two `request_reconcile` calls within `reconcile_request_min_interval_s`; a third within `reconcile_backoff_s` of a probe; then a `POLICY_ACTIVATE` of a fresh v2 bundle **between** a probe and a fourth call still inside the backoff window | second: `accepted, coalesced: true`, same `outcome_ref`, exactly one probe at the target (target-side call count is the observable); third: `RECONCILE_BACKOFF` with `retry_after_s`; fourth: still `RECONCILE_BACKOFF` — the activation neither resets nor re-keys the K7-anchored windows (guard-bite: include the policy digest in a dedupe key → the fourth call probes, control fails) |
| O11 | snapshot position coherence and non-upgrade (M2) | interleave writes with `observe_*` calls; capture `(store_authority_ref, snapshot_position, observed_at)` triples; skew the service wall clock backwards between two observations (test hook) so `observed_at` ordering inverts | `snapshot_position` is non-decreasing in true commit order within one `store_authority_ref` regardless of the `observed_at` inversion; two observations in one response never mix positions; ordering the two skewed observations by `snapshot_position` matches store history exactly; guard-bite: order by `observed_at` → the inverted pair misorders, proving the M2 rule is load-bearing |
| O12 | K1 exclusion | with `FULL` projection and `ANY` scope, attempt every expressible call for bundle content: no method accepts a policy/bundle ref; inspect every `ObservationV1` for K1 data | K1 appears only as `basis.active_policy {policy_id, revision, content_digest, seq}`, byte-equal to the activation row; no bundle bytes, no recomputed bundle digest, no bundle-content diagnostic anywhere in any response |
| O13 | observer scope and projection (M3; v2 active) | observer A: default entry (no `entitlements`); observer B: `observe_scope = [WORK_RUNS_STARTED_BY(P1)]`, `record_projection: FULL`; B reads (i) an in-horizon run, (ii) an out-of-horizon run started under P2, (iii) a nonexistent run, (iv) `observe_evidence` on an existing out-of-horizon envelope and on a nonexistent id | A: `observe_effects`/`observe_evidence_by_subject` succeed, `observe_effect`/`observe_evidence` → `PROJECTION_FORBIDDEN` (default is least-content); B: (i) full records; (ii) and (iii) **byte-identical `NOT_PRESENT`** (zero-row complete enumeration, §5.2 (b)) with horizon-qualified absence basis — out-of-horizon vs nonexistent indistinguishable (no existence oracle); (iv) both `NOT_PRESENT` with horizon-qualified basis; guard-bite: refuse out-of-horizon with `FORBIDDEN_FOR_SUBJECT` → (ii)/(iii) become distinguishable, control fails |
| O14 | multi-open-admission requester pass (v2 active) | force an effect into two open admissions (C9-style ordinal 2 after `UNKNOWN`); one accepted `request_reconcile` | exactly one probe **per open admission**, each writing one `:reconciler:requested` K7 row; dedupe/backoff windows anchored per effect (an immediate second request coalesces, not one-per-admission); per-admission exhaustion counts kernel-origin rows of that admission only |
| O15 | read→K7 crash window | crash-inject the kernel after the adapter probe returns and before the outcome row commits (kernel-origin and requester-origin legs); restart | no K7 row for the interrupted probe; no lock held after restart; restart scan re-reconciles the open admission; total recorded attempts never exceed actual probes; `RECONCILE_EXHAUSTED` seals only at `reconcile_max_attempts` **recorded** kernel-origin rows — never earlier than the bound (conservative direction proven) |
| O16 | observer-vs-writer liveness | hold one `observe_*` read transaction open at the reference deadline bound while running a full admission+dispatch cycle and a reconcile pass on the same store | the admission path completes without waiting on any observer-held lock (target-side effect lands; K6/K7 rows commit); the long read either completes on its snapshot or returns `UNKNOWN(READ_DEADLINE_EXCEEDED)`; guard-bite: route the observer read through serialization-domain lock `D` → admission stalls behind the reader, control fails |

### 8.1 Review control-debt disposition (canonical Review `issuecomment-5533914618` debt list, item-for-item)

| Debt item | Control |
|---|---|
| snapshot coherence | O11 (single-position responses; monotonicity per `store_authority_ref`) |
| missing-id absence | O3 (i)/(ii) |
| read-freshness non-upgrade | O11 (skewed-clock ordering leg + `observed_at` guard-bite) |
| exact effect enumeration | O3 (v) |
| K1 exclusion | O12 |
| zero-state-delta successful observation | O9 |
| negative workflow own-scope | O7 (workflow cross-request leg) |
| conclusive / no-open-admission reconcile refusal | O6 (`CONCLUSIVE` and `NO_OPEN_ADMISSION` legs) |
| dedupe/backoff activation behavior | O10 (mid-window `POLICY_ACTIVATE` leg + digest-in-key guard-bite) |
| multi-open-admission behavior | O14 |
| read→K7 crash window | O15 |
| bundle validation | O5 (both directions, incl. entitlement/bounds/unknown-key defects) |
| observer-vs-writer liveness | O16 |

No debt item is left undispositioned; none is deferred to implementation discretion.
...
**TD delta (exact section list for the integration edit):** §5.4 (add `cadp.kernel-config.v2` with the four new policy keys + entitlements sub-schema for `reconcile_scope` / `observe_scope` / `record_projection`; v1 retained; §7.4 future-version continuity rule), §6.5 (Reconciler: request intake ordering §6.4, per-effect gate anchoring with per-admission exhaustion, origin-tagged accounting, exhaustion floor, crash-window rule, §6.3 locks), §12 (four new methods; two new classes; matrix rewritten at `process_class` granularity per the §3.2 shape note, every cell explicit; structured `request_reconcile` response), §13.1 (O1–O16), §15 (record the replica/export read path and any further read surface as explicitly out of scope / future TD rounds). No change to §§1–4, §§7–11, §14 — in particular **no §0/§1/§3.1/§3.2 store-map change (M6): no reservation ledger or other operational table exists in this design**. **Kernel primitive delta: NONE** — no new K-record, no new signed document kind, no new incident kind, no new store table (existing tables and the landed subject / work-run indexes suffice; `ObservationV1` is never stored; the SQLite `snapshot_position` reference binding is derived from existing rowids, §4.2). **API delta:** +4 methods, +2 `process_class` values, `request_reconcile` request/response shape extended (`ack` kept as an alias of `accepted` for one generation), one existing reach cell narrowed (workflow `request_reconcile` → own scope). **Every element of this API delta is version-gated on the active bundle schema (§6.4/§7.2): while a v1 bundle is active, `observe_*` deterministically refuses and `request_reconcile` behaves byte-for-byte as landed.**
...
| M1–M8 (Major) | **DISPOSITIONED — §10.2.** The evidence blocker is resolved: the issue body now mirrors the full M1–M8 finding bodies from the canonical Review (`#106 issuecomment-5533914618`; mirror is transport, source canonical). Each finding is dispositioned against exact clauses and controls in §10.2, and the related control debt item-for-item in §8.1. |

### 10.1 Second-round Review closure (REQUEST_CHANGES on head `7c9e49b`)

| Finding | Disposition |
|---|---|
| F1 — M1–M8 undispositioned yet `TD_DESIGN_READY` | Accepted. In round 2, retrieval was re-verified impossible and the round returned `STOP_DESIGN_REQUIRED` as the finding directs. **Resolved in round 3:** Control mirrored the full M1–M8 bodies into the issue (transport mirror; source canonical), and every finding is now individually dispositioned in §10.2 with the control debt in §8.1, so `TD_DESIGN_READY` is claimed on evidenced closure (§12). |
...
### 10.2 Major findings M1–M8 disposition (round 3; bodies per the issue mirror of `issuecomment-5533914618`)

| Finding | Disposition |
|---|---|
| M1 — requester exhaustion floor | **Closed by a superseding mechanism, strictly stronger than the requested repair (§6.4).** The `reconcile_request_reserved_attempts` field the finding bounds is **withdrawn** — no reserved-floor key exists in v2. Origin-tagged accounting makes the Review's requested guarantee hold in degenerate form: external requests consume at most **zero** of `reconcile_max_attempts` (effective reserve = the whole budget, inside the required `ceil(max/2)..max` range); `RECONCILE_EXHAUSTED` seals only on kernel-origin `UNKNOWN` rows, so no requester sequence reaches the Human exception branch. The relocated harm (target-probe consumption against a slow target) is bounded trusted-side by policy-bound `reconcile_request_max_attempts` + dedupe + backoff. Falsified by O8 (incl. origin-split guard-bite). |
| M2 — no monotonic snapshot position | **Closed by adoption (§4.2).** `ObservationV1` gains required `snapshot_position` (monotonic per `store_authority_ref`; PostgreSQL LSN / SQLite derived data-version equivalent) plus `store_authority_ref`; `observed_at` is normatively presentation metadata and **never an ordering input**; the #96 `STUCK_SUSPECTED`-style comparison must order by `snapshot_position` within one `store_authority_ref`, else `UNKNOWN`. `AbsenceBasisV1.as_of` carries the position. Falsified by O11 (clock-skew inversion + `observed_at`-ordering guard-bite). |
| M3 — first bulk K2 read has no scope/content posture | **Closed by adoption (§4.4, §7.1).** Posture stated explicitly: bulk observation reach is a strict superset of landed read reach and a deliberate per-principal grant, never a default. Adopted `record_projection: FULL \| REFS_AND_DIGESTS_ONLY` **defaulting to `REFS_AND_DIGESTS_ONLY`** (realized as method gating so the closed result-type vocabulary of F3 survives — no redacted record variant exists) and optional `observe_scope` (the Review's scope-registry list) with horizon-uniform responses that create no existence oracle. Falsified by O13. |
| M4 — read safety bounds are build-time constants | **Closed by adoption, one reasoned variance (§4.3 rule 5, §7.1).** `observe_max_records_per_query` and `observe_deadline_ms` are required, bounded, reference-valued **policy keys** under the §5.4 no-implementation-defaults rule; build-constant phrasing is removed; two builds under one policy digest cannot differ on the load-bearing `complete` answer (stated normatively). Variance: `observe_max_queries_per_request` is not adopted because no multi-query request exists for it to govern — a dead key would itself violate §5.4; a future batch API round must add it (§7.1). |
| M5 — backoff anchor / epoch undefined | **Closed by withdrawal of the undefined constructs + an exact anchor rule (§6.4).** The reservation-row and "policy-owned backoff epoch" constructs no longer exist; the sole dedupe/backoff anchor is the `observed_at` of reconciler-authored K7 rows of the effect (the Review's offered alternative, degenerate since no reservation rows exist). There is no request key; the policy digest participates in **no** dedupe/backoff computation and survives as audit context only, so `POLICY_ACTIVATE` cannot reset or re-key the windows. Falsified by O10 (mid-window activation leg + digest-in-key guard-bite). |
| M6 — reservation ledger placement contradicts TD store invariants | **Closed by withdrawal (§6.4, §9).** The reservation ledger is gone: refusals write nothing; all accounting rides on constitutional K7 rows that already exist in the landed store. No operational table enters §3.1, no §0/§1/§3.2 store-map amendment is needed, and no constitutional/operational split is introduced — the finding's target mechanism has no successor in this design. The `snapshot_position` reference binding is likewise derived without a new table (§4.2). |
| M7 — reach matrix is incomplete for existing classes | **Closed by adoption (§3.2).** The matrix now fills the complete `observe_*` rows across **every** existing caller class plus root (all explicitly `no` — granting `workflow` the read API would recreate B1/B5 co-location) and the complete new-class columns across every landed method. The §12 integration-edit shape is stated exactly: the landed table is methods × five caller-category columns with `evidence-adapter`/`deployment-control` sharing a landed column header; the edit rewrites it at measured `process_class` granularity preserving every landed cell. Falsified by O1 (full cross-product + guard-bite). |
| M8 — `EVIDENCE_BY_SUBJECT` predicate is unpinned | **Closed by adoption (§4.1).** The query subject is pinned to exact string equality on `(authority_ref, namespace, object_id)` — the landed subject-index predicate — with a closed query schema: `revision_or_version` / `content_digest` must not be supplied and their presence is a malformed-request rejection, not wildcard or require-absent semantics. Digest precision is exact-ref only (`observe_evidence` + `expected_envelope_digest`). Zero-row `complete: true` is therefore meaningful (O3, O5 (ii) malformed-field leg). |
...
```text
DESIGN_DISPOSITION   TD_DESIGN_READY — one complete superseding TD amendment candidate.
                     All round-2 Blocking findings (T1–T8 scope) and all Major findings M1–M8
                     are individually dispositioned against exact clauses and controls (§10, §10.2),
                     the Review control-debt list is dispositioned item-for-item (§8.1), and the
                     T1 continuity contract conforms to the Human authority decision
                     #106 issuecomment-5540963810 (non-flag-day, explicit versioned schema; §7.4).
                     Fixed judgments preserved: SPEC_CHANGE=NO (re-verified §9), TD_GAP=CONFIRMED
                     (re-measured §1), Option A, K1 exclusion, no new Core role, no new authority
                     primitive. No fresh Spec/TD authority contradiction was found applying the
                     Human decision.
NEXT_OWNER           Control (scope/authority check) -> independent Review -> Human TD
...
| 10 | falsification controls | §8 (O1–O16), §8.1 (control-debt map) |

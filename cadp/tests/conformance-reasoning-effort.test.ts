...
test("only the codex reviewer currently requests measured effort", () => {
  ...
  for (const provider of ["claude", "grok"] as const) {
    assert.equal(REVIEW_PROVIDERS[provider].requested_effort, undefined);
    assert.equal(REVIEW_PROVIDERS[provider].effort_argv, undefined);
  }

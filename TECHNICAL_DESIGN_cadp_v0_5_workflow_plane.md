...
**Observed completeness floor (defense in depth).** The PEP already walks `candidate_base_sha..candidate_sha` when it verifies that the `GIT_PUSH` bundle reproduces exactly `candidate_sha` (§3.4; TD v0.4 §6.6, §8.1). The platform records the commit author/committer identity strings observed in that walk as an `INDEPENDENT_OBSERVATION` evidence item whose producer is the platform (PEP) itself — the platform is the producer of the *observation*. What the observed strings assert is another matter, stated honestly: commit authorship is producer-asserted content — anyone can commit under any name — so an observed identity string is **never identity proof**. Its use is one-directional: the evaluator takes the separation basis as the **union** of the attested `producers[]` and the observed authorship identities mapped through the policy identity registry (TD v0.4 §9.1), so an unreported co-producer who is visible in the authorship trail still blocks their own review even though the attestation omitted them. A co-producer whose authorship trail is forged or absent is not caught by this floor — that residual is covered only by the complete-set attestation accountability above, and the document marks it as residual rather than closed. Ordering: this observation comes into existence at `GIT_PUSH` bundle verification, so per this section's prerequisite matrix it can gate `PR_CREATE` and `PR_MERGE` but never `GIT_PUSH` itself, whose pre-push independence rests on the attested set alone.
...
14. **Observed-authorship refusal:** a reviewer absent from the attested `producers[]` but matching, through the policy identity registry, an author/committer identity observed in the PEP's `GIT_PUSH` bundle walk is refused (§4.2 observed completeness floor).
...
| `CANDIDATE_PROVENANCE` as ordinary evidence; complete `producers[]` set, empty set fails closed; union with observed authorship | Spec v0.5 §2.5, §4.5, K2 (attributable attestation); TD v0.4 §8.3 (`count(implementer_refs) > 0`), §8.4 (identity-class predicate), §9.1 (ingress discipline, no promoted self-report) |

...
- Reference: a second product surface (measured #90: Claude Code plan-mode, read-only, network denied) reviewing the exact committed diff at `candidate_sha`; the review adapter emits `REVIEW` evidence with `subject_bindings = [repo_id + candidate_sha]`, `claim = { verdict, body_digest, body_cas_key, reviewer_product, reviewer_run_id }`, `producer_ref` = reviewer identity.
- **Reviewer body retention (#259 P0b).** `body_digest` and `body_cas_key` are a **self-verifying pair**: the reviewer's full stdout bytes are put into CAS and the digest is computed over exactly those stored bytes, so any later reader can fetch by `body_cas_key`, re-digest, and compare. The Ingress enforces the pair on every PRESENT `REVIEW` (fetch, re-digest, compare **before** the seal — `REVIEW_CLAIM_INVALID` otherwise), so a sealed review is always one whose findings can still be quoted byte-for-byte after the run stops. The body is stored **as-is**: no truncation and no redaction — it is the reviewer surface's own text, carries no kernel secret, and the broker's 60 000-character cap bounds the reviewer **prompt**, never this evidence. The operator recovery path is `readReviewBody` (`cadp/live/reviewBody.ts`, surfaced as `ctl review-body <dir> <evidence_id>`): read-only, evidence-id-scoped — not a generic blob API — and it re-verifies the pair before emitting a byte.
...
| `REVIEW` | GitHub PR review / second-surface reviewer | `repo_id + commit_id` | `cadp.review.v1` `{verdict, body_digest, body_cas_key, reviewer_run_id?}` — `body_cas_key` names the CAS object holding the reviewer's full text and `body_digest` is the sha256 of exactly those bytes (§8.3; Ingress-enforced, no truncation). Identity class derived from `producer_ref`, §9.1 | `INDEPENDENT_OBSERVATION` or `SELF_REPORT` (if producer == implementer) |
...
**REVIEW.** `reviewCandidate` submits `REVIEW` as `reviewer:claude-code` / `reviewer:grok` / `reviewer:codex` with `cadp.review.v1` `{ verdict, body_digest, body_cas_key, reviewer_run_id }`. It first puts the reviewer's stdout bytes into CAS with the SAME reviewer principal that seals the envelope, then takes both `body_digest` and `body_cas_key` from that one buffer, so the pair is self-verifying over exactly the stored bytes (§8.3, #259 P0b); each of the workflow's two review rounds stores its own body, and the stopped run's trace names both (`round_<n>_review_evidence_id`). The chosen product is the kernel-stamped `producer_ref` (identity_class derived from the active `identity_registry`, never from the claim). The claim carries no observed model and no effort. `brokerReview` calls `runReviewer` with `{ workspace, auth, argv }` only.

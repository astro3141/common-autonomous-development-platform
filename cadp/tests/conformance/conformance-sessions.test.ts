...
import { FAILED_SESSION_RETENTION, VERIFIER_TEST_ARGV, preserveFailedSession } from "../../product/surfaceBroker.ts";
...
test("runVerifier carries the broker's PINNED test argv, isolation profile otherwise unchanged", async () => {
  // The argv is `VERIFIER_TEST_ARGV`, not a literal repeated here: the executed set is pinned by a
  // gate-protected file (`surfaceBroker.ts`), never by the delegable `npm test` script. Narrowing
  // the pin — dropping the explicit path, or pointing it at a subdirectory — fails this test and
  // GF8 in conformance-gatefiles.test.ts.
